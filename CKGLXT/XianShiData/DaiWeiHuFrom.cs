...
{
    public partial class DaiWeiHuFrom : BaseFuFrom
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (daixiu<=0)
            {
                this.QiDongTiShiKuang("请输入待修数量");
                return;
            }
            if (daixiu> shuju[0])
            {
                this.QiDongTiShiKuang("输入待修数量大于可用数量");
                return;
            }
        ...
        private void GaiBianZhuanTai(int keyongshuliang, int daixiu, int yiqianjiechu)
        {
            ...
            using (AddOrUpdate orUpdate = new AddOrUpdate())
            {
                ...
                orUpdate.ZengJiaSql("HWWHCount", yiqianjiechu + daixiu, ZiFuOrInt.Int);
